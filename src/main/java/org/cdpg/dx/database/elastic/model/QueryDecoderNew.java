...
public class QueryDecoderNew {
  ...
  public QueryModel getQueryModel(QueryDecoderRequestDTO request) {
    ...
    if (searchType != null && searchType.matches(SEARCH_CRITERIA_REGEX)) {
      ...
      new SearchCriteriaQueryDecoratorNew(queryMap, request.getSearchCriteriaRequest()).add();
      isValidQuery = true;
    }

    if (searchType != null && searchType.matches(TEXTSEARCH_REGEX)) {
      LOGGER.debug("Info: Text search block");
      new TextSearchQueryDecoratorNew(queryMap, request.getTextSearchRequest()).add();
      isValidQuery = true;
    }

    new AccessPolicyQueryDecoratorNew(queryMap, request.getAccessPolicyRequest()).add();
    ...
    if (searchType != null && searchType.matches(RESPONSE_FILTER_REGEX)) {
      new ResponseFilterDecoratorNew(queryMap, request.getResponseFilterRequest()).add();
      isValidQuery = true;
    }
  ...
  public QueryModel listMultipleItemTypesQuery(QueryDecoderRequestDTO request) {
    ...
    new AccessPolicyQueryDecoratorNew(queryMap, request.getAccessPolicyRequest()).add();
    new SearchCriteriaQueryDecoratorNew(queryMap, request.getSearchCriteriaRequest()).add();
    new InstanceFilterQueryDecoratorNew(queryMap, request.getInstanceFilterRequest()).add();
